...
class MultilingualTextSimilarityChecker:
    def __init__(self,min_threshold = 0.7,max_threshold = 0.8,steps = 100):
        # 加载多语言语义相似度模型
    ...
    def is_similar(self, respnse, ground_truth,threshold):
        results = self.get_similarity_score(respnse, ground_truth)
        return results['final_score'] >= threshold





# def accuracy_reward(response: str, ground_truth: str,checker : MultilingualTextSimilarityChecker) -> float:
