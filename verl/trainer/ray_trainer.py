...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    advantages, returns, weights = compute_advantage_return(adv_estimator, **adv_inputs)
    data.batch["advantages"] = advantages
    data.batch["returns"] = returns
    data.batch["weights"] = weights
    return data
...
class RayPPOTrainer:
    ...
    def init_workers(self) -> None:
        ...
        if self.use_reward_model:
            # we create a RM here
            print('*'*90)
            print(Role.RewardModel)
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.RewardModel)
            rm_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.RewardModel], config=self.config.worker, role="reward"
            )
            self.resource_pool_to_cls[resource_pool]["rm"] = rm_cls
        print(self.resource_pool_to_cls)
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                batch.meta_info['global_step'] = self.global_step -1
