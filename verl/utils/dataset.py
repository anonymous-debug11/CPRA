...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.prealign_ratio = prealign_ratio
        self.train = False
        if "@" in data_path:
        ...
        if os.path.isdir(data_path):
            ...
            self.dataset = DatasetDict.load_from_disk(data_path)[data_split]
    ...
    def __getitem__(self, index):
        ...
        if 'time' in example:
            ...
            example['time'] =1
        messages = self._build_messages(example)
        ans = example.pop(self.answer_key)
        # if len(ans) >2:
        #     # 计算当前时间步数的长度比例
        #     # ratio = 1+self.time / self.__len__() *( 2 * self.__len__()/(2-2*self.__len__())) 
        #     # 计算当前实例的处置长度
        #     length = len(ans)-example['time'] if len(ans)-example['time']>0 else 0
        # else:
        #     length = 0

        if len(ans) >2:
            # 计算当前时间步数的长度比例
            ratio = 1-self.time*self.prealign_ratio/self.__len__()
            # 计算当前实例的处置长度
            length = int(ratio * len(ans)) if int(ratio * len(ans)) >=0 else 0
        else:
            length = 0
        if not self.train:
            length = 0
        ...
        example["ground_truth"] = ans_a# example.pop(self.answer_key)
        return example
